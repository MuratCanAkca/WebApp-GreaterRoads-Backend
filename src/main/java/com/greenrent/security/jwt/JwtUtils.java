...
public class JwtUtils {
	...
	public String generateJwtToken(Authentication authentication) {
		...
		return Jwts.builder().
				...
				setExpiration(new Date(new Date().getTime()+jywExpirationMs)).
				signWith(SignatureAlgorithm.HS512, jwtSecret).
				compact();
		//generate token olusturma kodu
	}
	
	public Long getIdFromJwtToken(String token) {
		String strId=Jwts.parser().setSigningKey(jwtSecret).parseClaimsJws(token).getBody().getSubject();
		return Long.parseLong(strId);
		//gelen tokenden ıd extract etmek
	}
	...
	public boolean validateJwtToken(String token) {
		try {
			...
			logger.error("JWT Token is unsupperted",e.getMessage());
